...
macOS 优先的 Codex Desktop 手机桥接 MVP。电脑侧运行 Rust sidecar，手机侧打开同一局域网内的 PWA，用来查看会话流、发送文本回复，并处理可捕获的审批请求。

## MVP 范围

- 桌面端优先支持 Codex Desktop App，通过 CDP target 发现和注入 bridge 脚本连接 app-server JSON-RPC。
...
桌面壳当前是产品化 Mac App 的 scaffold：提供 Codex 检测/启动、Bridge start/stop、手机配对链接、Quick Tunnel Beta、设备撤销和本地诊断入口。开发模式会自动向上定位仓库根目录，并默认读取：
...
Codex Desktop 必须以 remote debugging port 启动，端口和 `CODEX_MOBILE_BRIDGE_DEBUG_PORT` 一致。当前 MVP 默认端口是 `9229`。
...
- `codex_not_running`：sidecar 尚未跑过有效诊断，或 Codex 未启动。
- `cdp_unavailable`：debug port 不可达，检查 Codex 启动参数和端口。
...
1. 启动带 remote debugging port 的 Codex Desktop。
